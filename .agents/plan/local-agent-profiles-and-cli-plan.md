# Subagent profiles and Hearth agent CLI plan

## Decision

Subagent profiles describe roles over built-in coding-agent providers.
Hearth owns provider invocation and lifecycle. Custom CLI-backed agents,
...
```bash
hearth agents ls
hearth agents run <profile-or-id> "<prompt>"
hearth agents show <id>
```

Profile discovery happens through the compact catalog returned by
`open_workspace`. `hearth agents ls` lists existing subagent sessions for the
...
Hearth maps provider ids to native integrations:
...
Shell calls launched through Hearth receive `HEARTH_WORKSPACE_ID` and
`HEARTH_WORKSPACE_ROOT`, so `hearth agents ls` can scope itself without the
